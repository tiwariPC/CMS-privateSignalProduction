...
cd /afs/cern.ch/cms/PPD/PdmV/work/McM/submit/NPS-BBDM-RunIII2024Summer24DRPremix/
...
# Dump actual test code to a NPS-BBDM-RunIII2024Summer24DRPremix_test.sh file that can be run in Singularity
cat <<'EndOfTestFile' > NPS-BBDM-RunIII2024Summer24DRPremix_test.sh
#!/bin/bash
...
chmod +x NPS-BBDM-RunIII2024Summer24DRPremix_test.sh
...
singularity run --no-home /cvmfs/unpacked.cern.ch/registry.hub.docker.com/cmssw/$CONTAINER_NAME $(echo $(pwd)/NPS-BBDM-RunIII2024Summer24DRPremix_test.sh)

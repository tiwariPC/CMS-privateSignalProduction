...
2. Update the `privateSignalProduction.submit` and `privateSignalProduction.sh` files.
    ...
    1. Add the appropriate number of events and jobs. For example:
        1. If you want 50k events then you can change `Queue 50` in the submit file and put 1000 in each python configuration files.
3. To create ```.sh``` and ```.submit``` file for the condor submission
